...
message DriverCreateBucketResponse {
    ...
    // REQUIRED: Tt least one protocol MUST return non-nil.
    // OPTIONAL: More than one protocols MAY return non-nil if the Provisioner can support access to
    //           the bucket via multiple protocols.
    // Bucket info for each protocol MAY be stored plaintext for COSI administrators to assist in
    // debugging configuration errors. Bucket info is not generally considered sensitive. It is
    // RECOMMENDED to return all available info to in administrator debugging. The Provisioner MAY
    // omit any or all bucket info fields for any supported protocol if desired.
