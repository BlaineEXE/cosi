...
// S3 bucket info environment variables.
type S3BucketInfoVar BucketInfoVar

// S3 access credential environment variables.
type S3CredentialVar CredentialVar

// bucket info vars
const (
	// Required. The S3 bucket ID as used by clients.
	BucketInfoVar_S3_BucketId S3BucketInfoVar = "COSI_S3_BUCKET_ID"

	// Required. The S3 endpoint for the bucket.
	BucketInfoVar_S3_Endpoint S3BucketInfoVar = "COSI_S3_ENDPOINT"

	// Required. The S3 region for the bucket.
	BucketInfoVar_S3_Region S3BucketInfoVar = "COSI_S3_REGION"

	// Required. The S3 addressing style. One of `path` or `virtual`.
	// See: https://docs.aws.amazon.com/AmazonS3/latest/userguide/VirtualHosting.html.
	BucketInfoVar_S3_AddressingStyle S3BucketInfoVar = "COSI_S3_ADDRESSING_STYLE"
)
...
const (
	// S3AddressingStylePath represents path-style S3 addressing.
	S3AddressingStylePath = "path"

	// S3AddressingStyleVirtual represents virtual-hosted-style S3 addressing.
...
// Azure bucket info environment variables.
type AzureBucketInfoVar BucketInfoVar

// S3 access credential environment variables.
type AzureCredentialVar CredentialVar

// bucket info vars
const (
	// Required. The ID of the Azure storage account.
	BucketInfoVar_Azure_StorageAccount AzureBucketInfoVar = "COSI_AZURE_STORAGE_ACCOUNT"
)
...
const (
	...
	CredentialVar_Azure_AccessToken AzureCredentialVar = "COSI_AZURE_ACCESS_TOKEN"

	// Optional. The timestamp when access will expire.
	// Empty if unset. Otherwise, date+time in ISO 8601 format.
	CredentialVar_Azure_ExpiryTimestamp AzureCredentialVar = "COSI_AZURE_EXPIRY_TIMESTAMP"
)
...
// Google Cloud Storage (GCS) bucket info environment variables.
type GcsBucketInfoVar BucketInfoVar

// Google Cloud Storage (GCS) bucket info environment variables.
type GcsCredentialVar CredentialVar

// bucket info vars
const (
	// Required. The GCS project ID.
	BucketInfoVar_GCS_ProjectId GcsBucketInfoVar = "COSI_GCS_PROJECT_ID"

	// Required. GCS bucket name as used by clients.
	BucketInfoVar_GCS_BucketName GcsBucketInfoVar = "COSI_GCS_BUCKET_NAME"
)

// nolint:gosec // credential vars, not hardcoded credentials
const (
	// Required for `AuthenticationType=Key`. HMAC access ID.
	CredentialVar_GCS_AccessId GcsCredentialVar = "COSI_GCS_ACCESS_ID"

	// Required for `AuthenticationType=Key`. HMAC secret.
	CredentialVar_GCS_AccessSecret GcsCredentialVar = "COSI_GCS_ACCESS_SECRET"

	// GCS private key name.
	CredentialVar_GCS_PrivateKeyName GcsCredentialVar = "COSI_GCS_PRIVATE_KEY_NAME"

	// GCS service account name.
	CredentialVar_GCS_ServiceAccount GcsCredentialVar = "COSI_GCS_SERVICE_ACCOUNT"
)
